...
local save_manager = {}
save_manager.folder = 'Bracket'

function save_manager:setup_folders()
    local paths = {
        self.folder,
        self.folder..'/configs' 
    }
...
function save_manager:get_configs()
...
function save_manager:save_config(name)
    local path = self.folder..'/configs/'..name..'.json'
    local data = {}
    writefile(path, game:GetService('HttpService'):JSONEncode(data))
end

function save_manager:load_config(name)
    local path = self.folder..'/configs/'..name..'.json'
    if not isfile(path) then return end
    local data = game:GetService('HttpService'):JSONDecode(readfile(path))
end

local config_tab = WinTypes:CreateTab("Configuration")
local config_section = config_tab:CreateGroupbox("Configs", "Left") 

local config_name = config_section:CreateTextBox("Config Name", "", function() end)
local config_list = config_section:CreateDropdown("Saved Configs", save_manager:get_configs(), function(cfg)
    if cfg then
        config_name:SetValue(cfg)
    end
end)

config_section:CreateButton("Save", function()
    local name = config_name:GetValue()
    if name ~= "" then
        save_manager:save_config(name)
        config_list:SetOption(save_manager:get_configs())
    end
end)

config_section:CreateButton("Load", function()
    local name = config_name:GetValue()
    if name ~= "" then
        save_manager:load_config(name)
    end
end)

save_manager:setup_folders()
    core.Name = "core"
    ...
    core.InputBegan:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseMovement or Enum.UserInputType.Touch or Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.Touch) and userinputservice:GetFocusedTextBox() == nil then
        ...
        function TabTypes:CreateGroupbox(name, side)
            ...
            function GroupTypes:CreateSlider(name, min, max, def, callback)
                ...
                slider.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseMovement or Enum.UserInputType.Touch or Enum.UserInputType.Touch then
                        Slide(input)
                        Dragging = true
                        SliderDragging = true
                    end
